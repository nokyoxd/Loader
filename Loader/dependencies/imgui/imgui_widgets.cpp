...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    auto it_hover = anim.find(id);
    if (it_hover == anim.end())
    {
        anim.insert({ id, 0.f });
        it_hover = anim.find(id);
    }
    if (*v)
    {
        if (it_hover->second != 1.f)
        {
            if (it_hover->second < 1.f)
                it_hover->second = ImMin(it_hover->second + deltatime, 1.f);
            else if (it_hover->second > 1.f)
                it_hover->second = ImMax(1.f, it_hover->second - deltatime);
        }
    }
    else
    {
        if (it_hover->second != 0.f)
        {
            if (it_hover->second < 0.f)
                it_hover->second = ImMin(it_hover->second + deltatime, 0.f);
            else if (it_hover->second > 0.f)
                it_hover->second = ImMax(0.f, it_hover->second - deltatime);
        }
    }
    ...
    if (mixed_value)
    {
        ...
        window->DrawList->AddRectFilled(check_bb.Min + pad, check_bb.Max - pad, ImColor(0.7647058823529412, 0.f, 0.3215686274509804f, it_hover->second), style.FrameRounding);
    }
    else if (*v)
    {
        ...
        // checkbox
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImColor(0.7647058823529412, 0.f, 0.3215686274509804f, it_hover->second), style.FrameRounding);
        // gradient
        window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.50f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.50f)));
    }
    ...
    if (label_size.x > 0.0f)
    {
        if (*v)
        {
            RenderTextActive(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y - 1), label);
        }
        else
        {
...
bool ImGui::Category(const char* label, const ImVec2& size_arg, bool state, int x, int y)
{
    ...
    /*ImGui::RenderTextClipped(bb.Min, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);*/
    ...
    if (state)
    {
        pWindowDrawList->AddRectFilledMultiColor(bb.Min, ImVec2(bb.Max.x * it_hover->second, bb.Max.y), ImColor(0.7647058823529412f, 0.f, 0.3215686274509804f, 0.1372549019607843f), ImColor(0.f, 0.f, 0.f, 0.f), ImColor(0.f, 0.f, 0.f, 0.f), ImColor(0.7647058823529412f, 0.f, 0.3215686274509804f, 0.1372549019607843f));
        pWindowDrawList->AddRectFilled(bb.Min, bb.Min + ImVec2(2, 30), ImColor(0.7647058823529412f, 0.f, 0.3215686274509804f, 1.f * it_hover->second), 1.f, 0);
        pWindowDrawList->AddText(bb.Min + ImVec2(x, y), ImColor(0.7647058823529412f, 0.f, 0.3215686274509804f, 1.f * it_hover->second), label);
    }
    else
    {
        pWindowDrawList->AddText(bb.Min + ImVec2(x, y), ImColor(0.7843137254901961f, 0.7843137254901961f, 0.7843137254901961f, 0.392156862745098 - ( 0.392156862745098 * it_hover->second )), label);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float expected_w = CalcItemWidth() + 62;
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : expected_w;
    const float h = GetFrameHeight() + 5;

    // modified by qo0
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + h + 1 : 0.0f, label_size.x > 0.0f ? label_size.y : 0.0f), window->DC.CursorPos + ImVec2(w, (label_size.x > 0.0f ? label_size.y : 0.0f) + h));
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 6), GetColorU32(ImGuiCol_Triangle), popup_open ? ImGuiDir_Up : ImGuiDir_Down, 0.5f);
    }
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + style.FramePadding + ImVec2(0, 2), ImVec2(value_x2, frame_bb.Max.y), preview_value, NULL, NULL, ImVec2(0.0f, 0.0f));
    if (label_size.x > 0)
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
        ...
        window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, ImColor(195, 0, 82, 35), ImColor(195, 0, 82, 0), ImColor(195, 0, 82, 0), ImColor(195, 0, 82, 35));
        window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Min.x + 3, bb.Max.y), ImColor(195, 0, 82, 255), 1.f, 0);
    }
